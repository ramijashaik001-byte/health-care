# app/api/allergen.py
"""
FastAPI Router defining REST API endpoints for the Allergen controller.
...
@router.get("/", response_model=AllergenListResponse, status_code=status.HTTP_200_OK)
def list_allergens(
    ...
    """Retrieve paginated lists of Allergen records."""
...
@router.get("/{entity_id}", response_model=AllergenResponse, status_code=status.HTTP_200_OK)
def get_allergen_by_id(entity_id: int, db: Session = Depends(get_db)):
    """Fetch a single Allergen record by unique ID."""
    service = AllergenService(db)
    return service.get_by_id(entity_id)

@router.post("/", response_model=AllergenResponse, status_code=status.HTTP_201_CREATED)
def create_allergen(obj_in: AllergenCreate, db: Session = Depends(get_db)):
    """Submit and register a new Allergen entity."""
    service = AllergenService(db)
    return service.create(obj_in)

@router.put("/{entity_id}", response_model=AllergenResponse, status_code=status.HTTP_200_OK)
def update_allergen(entity_id: int, obj_in: AllergenUpdate, db: Session = Depends(get_db)):
    """Update properties of an existing Allergen record."""
    service = AllergenService(db)
    return service.update(entity_id, obj_in)

@router.delete("/{entity_id}", response_model=AllergenResponse, status_code=status.HTTP_200_OK)
def delete_allergen(entity_id: int, db: Session = Depends(get_db)):
    """Remove a Allergen record from system data storage."""
    service = AllergenService(db)
    return service.delete(entity_id)

@router.post("/{entity_id}/verify-stage-0", status_code=status.HTTP_200_OK)
def verify_allergen_stage_0(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_0(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_1(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_1(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_2(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_2(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_3(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_3(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_4(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_4(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_5(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_5(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_6(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_6(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_7(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_7(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_8(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_8(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_9(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_9(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_10(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_10(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_11(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_11(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_12(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_12(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_13(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_13(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_14(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_14(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_15(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_15(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_16(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_16(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_17(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_17(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_18(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_18(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_19(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_19(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_20(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_20(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_21(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_21(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_22(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_22(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_23(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_23(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_allergen_stage_24(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_allergen_threshold_24(entity_id: int, value: float, db: Session = Depends(get_db)):
