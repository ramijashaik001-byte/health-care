# app/api/ward.py
"""
FastAPI Router defining REST API endpoints for the Ward controller.
...
@router.get("/", response_model=WardListResponse, status_code=status.HTTP_200_OK)
def list_wards(
    ...
    """Retrieve paginated lists of Ward records."""
    filters = WardFilter(
        search=search,
        limit=limit,
        offset=offset,
...
@router.get("/{entity_id}", response_model=WardResponse, status_code=status.HTTP_200_OK)
def get_ward_by_id(entity_id: int, db: Session = Depends(get_db)):
    """Fetch a single Ward record by unique ID."""
    service = WardService(db)
    return service.get_by_id(entity_id)

@router.post("/", response_model=WardResponse, status_code=status.HTTP_201_CREATED)
def create_ward(obj_in: WardCreate, db: Session = Depends(get_db)):
    """Submit and register a new Ward entity."""
    service = WardService(db)
    return service.create(obj_in)

@router.put("/{entity_id}", response_model=WardResponse, status_code=status.HTTP_200_OK)
def update_ward(entity_id: int, obj_in: WardUpdate, db: Session = Depends(get_db)):
    """Update properties of an existing Ward record."""
    service = WardService(db)
    return service.update(entity_id, obj_in)

@router.delete("/{entity_id}", response_model=WardResponse, status_code=status.HTTP_200_OK)
def delete_ward(entity_id: int, db: Session = Depends(get_db)):
    """Remove a Ward record from system data storage."""
    service = WardService(db)
    return service.delete(entity_id)

@router.post("/{entity_id}/verify-stage-0", status_code=status.HTTP_200_OK)
def verify_ward_stage_0(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_0(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_1(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_1(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_2(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_2(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_3(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_3(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_4(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_4(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_5(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_5(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_6(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_6(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_7(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_7(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_8(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_8(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_9(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_9(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_10(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_10(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_11(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_11(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_12(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_12(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_13(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_13(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_14(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_14(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_15(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_15(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_16(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_16(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_17(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_17(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_18(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_18(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_19(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_19(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_20(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_20(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_21(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_21(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_22(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_22(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_23(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_23(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ward_stage_24(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ward_threshold_24(entity_id: int, value: float, db: Session = Depends(get_db)):
