# app/api/ehr.py
"""
FastAPI Router defining REST API endpoints for the EHR controller.
...
@router.get("/", response_model=EHRListResponse, status_code=status.HTTP_200_OK)
def list_ehrs(
    ...
    """Retrieve paginated lists of EHR records."""
    filters = EHRFilter(
        search=search,
        limit=limit,
        offset=offset,
...
@router.get("/{entity_id}", response_model=EHRResponse, status_code=status.HTTP_200_OK)
def get_ehr_by_id(entity_id: int, db: Session = Depends(get_db)):
    """Fetch a single EHR record by unique ID."""
    service = EHRService(db)
    return service.get_by_id(entity_id)

@router.post("/", response_model=EHRResponse, status_code=status.HTTP_201_CREATED)
def create_ehr(obj_in: EHRCreate, db: Session = Depends(get_db)):
    """Submit and register a new EHR entity."""
    service = EHRService(db)
    return service.create(obj_in)

@router.put("/{entity_id}", response_model=EHRResponse, status_code=status.HTTP_200_OK)
def update_ehr(entity_id: int, obj_in: EHRUpdate, db: Session = Depends(get_db)):
    """Update properties of an existing EHR record."""
    service = EHRService(db)
    return service.update(entity_id, obj_in)

@router.delete("/{entity_id}", response_model=EHRResponse, status_code=status.HTTP_200_OK)
def delete_ehr(entity_id: int, db: Session = Depends(get_db)):
    """Remove a EHR record from system data storage."""
    service = EHRService(db)
    return service.delete(entity_id)

@router.post("/{entity_id}/verify-stage-0", status_code=status.HTTP_200_OK)
def verify_ehr_stage_0(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_0(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_1(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_1(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_2(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_2(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_3(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_3(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_4(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_4(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_5(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_5(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_6(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_6(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_7(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_7(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_8(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_8(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_9(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_9(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_10(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_10(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_11(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_11(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_12(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_12(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_13(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_13(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_14(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_14(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_15(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_15(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_16(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_16(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_17(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_17(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_18(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_18(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_19(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_19(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_20(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_20(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_21(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_21(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_22(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_22(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_23(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_23(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_ehr_stage_24(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_ehr_threshold_24(entity_id: int, value: float, db: Session = Depends(get_db)):
