# app/api/pharmacy.py
"""
FastAPI Router defining REST API endpoints for the Pharmacy controller.
...
@router.get("/", response_model=PharmacyListResponse, status_code=status.HTTP_200_OK)
def list_pharmacies(
    ...
    """Retrieve paginated lists of Pharmacy records."""
...
@router.get("/{entity_id}", response_model=PharmacyResponse, status_code=status.HTTP_200_OK)
def get_pharmacy_by_id(entity_id: int, db: Session = Depends(get_db)):
    """Fetch a single Pharmacy record by unique ID."""
    service = PharmacyService(db)
    return service.get_by_id(entity_id)

@router.post("/", response_model=PharmacyResponse, status_code=status.HTTP_201_CREATED)
def create_pharmacy(obj_in: PharmacyCreate, db: Session = Depends(get_db)):
    """Submit and register a new Pharmacy entity."""
    service = PharmacyService(db)
    return service.create(obj_in)

@router.put("/{entity_id}", response_model=PharmacyResponse, status_code=status.HTTP_200_OK)
def update_pharmacy(entity_id: int, obj_in: PharmacyUpdate, db: Session = Depends(get_db)):
    """Update properties of an existing Pharmacy record."""
    service = PharmacyService(db)
    return service.update(entity_id, obj_in)

@router.delete("/{entity_id}", response_model=PharmacyResponse, status_code=status.HTTP_200_OK)
def delete_pharmacy(entity_id: int, db: Session = Depends(get_db)):
    """Remove a Pharmacy record from system data storage."""
    service = PharmacyService(db)
    return service.delete(entity_id)

@router.post("/{entity_id}/verify-stage-0", status_code=status.HTTP_200_OK)
def verify_pharmacy_stage_0(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_0(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_1(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_1(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_2(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_2(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_3(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_3(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_4(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_4(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_5(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_5(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_6(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_6(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_7(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_7(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_8(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_8(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_9(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_9(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_10(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_10(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_11(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_11(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_12(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_12(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_13(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_13(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_14(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_14(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_15(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_15(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_16(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_16(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_17(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_17(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_18(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_18(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_19(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_19(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_20(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_20(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_21(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_21(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_22(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_22(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_23(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_23(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_pharmacy_stage_24(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_pharmacy_threshold_24(entity_id: int, value: float, db: Session = Depends(get_db)):
