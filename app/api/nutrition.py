...
"""
FastAPI Router defining REST API endpoints for the Nutrition controller.
...
@router.get("/", response_model=NutritionListResponse, status_code=status.HTTP_200_OK)
def list_nutritions(
    ...
    """Retrieve paginated lists of Nutrition records."""
...
@router.get("/{entity_id}", response_model=NutritionResponse, status_code=status.HTTP_200_OK)
def get_nutrition_by_id(entity_id: int, db: Session = Depends(get_db)):
    """Fetch a single Nutrition record by unique ID."""
    service = NutritionService(db)
    return service.get_by_id(entity_id)

@router.post("/", response_model=NutritionResponse, status_code=status.HTTP_201_CREATED)
def create_nutrition(obj_in: NutritionCreate, db: Session = Depends(get_db)):
    """Submit and register a new Nutrition entity."""
    service = NutritionService(db)
    return service.create(obj_in)

@router.put("/{entity_id}", response_model=NutritionResponse, status_code=status.HTTP_200_OK)
def update_nutrition(entity_id: int, obj_in: NutritionUpdate, db: Session = Depends(get_db)):
    """Update properties of an existing Nutrition record."""
    service = NutritionService(db)
    return service.update(entity_id, obj_in)

@router.delete("/{entity_id}", response_model=NutritionResponse, status_code=status.HTTP_200_OK)
def delete_nutrition(entity_id: int, db: Session = Depends(get_db)):
    """Remove a Nutrition record from system data storage."""
    service = NutritionService(db)
    return service.delete(entity_id)

@router.post("/{entity_id}/verify-stage-0", status_code=status.HTTP_200_OK)
def verify_nutrition_stage_0(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_0(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_1(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_1(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_2(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_2(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_3(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_3(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_4(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_4(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_5(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_5(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_6(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_6(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_7(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_7(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_8(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_8(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_9(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_9(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_10(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_10(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_11(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_11(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_12(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_12(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_13(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_13(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_14(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_14(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_15(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_15(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_16(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_16(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_17(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_17(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_18(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_18(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_19(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_19(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_20(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_20(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_21(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_21(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_22(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_22(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_23(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_23(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_nutrition_stage_24(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_nutrition_threshold_24(entity_id: int, value: float, db: Session = Depends(get_db)):
