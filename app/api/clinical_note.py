# app/api/clinical_note.py
"""
FastAPI Router defining REST API endpoints for the ClinicalNote controller.
...
router = APIRouter(prefix="/clinical_note", tags=["ClinicalNote"])

@router.get("/", response_model=ClinicalNoteListResponse, status_code=status.HTTP_200_OK)
def list_clinical_notes(
    ...
    """Retrieve paginated lists of ClinicalNote records."""
    filters = ClinicalNoteFilter(
        search=search,
        limit=limit,
        offset=offset,
        sort_by=sort_by,
...
@router.get("/{entity_id}", response_model=ClinicalNoteResponse, status_code=status.HTTP_200_OK)
def get_clinical_note_by_id(entity_id: int, db: Session = Depends(get_db)):
    """Fetch a single ClinicalNote record by unique ID."""
    service = ClinicalNoteService(db)
    return service.get_by_id(entity_id)

@router.post("/", response_model=ClinicalNoteResponse, status_code=status.HTTP_201_CREATED)
def create_clinical_note(obj_in: ClinicalNoteCreate, db: Session = Depends(get_db)):
    """Submit and register a new ClinicalNote entity."""
    service = ClinicalNoteService(db)
    return service.create(obj_in)

@router.put("/{entity_id}", response_model=ClinicalNoteResponse, status_code=status.HTTP_200_OK)
def update_clinical_note(entity_id: int, obj_in: ClinicalNoteUpdate, db: Session = Depends(get_db)):
    """Update properties of an existing ClinicalNote record."""
    service = ClinicalNoteService(db)
    return service.update(entity_id, obj_in)

@router.delete("/{entity_id}", response_model=ClinicalNoteResponse, status_code=status.HTTP_200_OK)
def delete_clinical_note(entity_id: int, db: Session = Depends(get_db)):
    """Remove a ClinicalNote record from system data storage."""
    service = ClinicalNoteService(db)
    return service.delete(entity_id)

@router.post("/{entity_id}/verify-stage-0", status_code=status.HTTP_200_OK)
def verify_clinical_note_stage_0(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_0(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_1(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_1(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_2(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_2(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_3(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_3(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_4(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_4(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_5(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_5(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_6(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_6(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_7(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_7(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_8(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_8(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_9(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_9(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_10(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_10(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_11(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_11(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_12(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_12(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_13(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_13(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_14(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_14(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_15(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_15(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_16(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_16(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_17(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_17(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_18(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_18(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_19(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_19(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_20(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_20(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_21(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_21(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_22(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_22(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_23(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_23(entity_id: int, value: float, db: Session = Depends(get_db)):
...
def verify_clinical_note_stage_24(entity_id: int, db: Session = Depends(get_db)):
...
def evaluate_clinical_note_threshold_24(entity_id: int, value: float, db: Session = Depends(get_db)):
